...
internals.transformSchema = Joi.object({
    transform: Joi.object({
        ...
        consume: Joi.array().items(Joi.string()).single().default([]),
        joi: Joi.object(),
...
internals.validateTransform = function (newTransform, existingTransforms) {
    ...
    // Ending period to deal with config.app.hot against config.app.hotdog
    const normalizeRoot = (root) => (root || '') + '.';

    const newRoots = newTransform.transform.consume.concat(newTransform.transform.root).map(normalizeRoot);
    ...
    for (let i = 0; i < existingTransforms.length; ++i) {
        ...
        if (existingBefore.indexOf(newTransform.transform.name) === -1 &&
            ...
            const existingRoots = existingTransform.transform.consume.concat(existingTransform.transform.root).map(normalizeRoot);

            newRoots.forEach((newRoot) => {

                existingRoots.forEach((existingRoot) => {

                    Hoek.assert(newRoot.indexOf(existingRoot) !== 0 && existingRoot.indexOf(newRoot) !== 0,
                        `Loveboat transform ${newTransform.transform.name}`,
                        `conflicts with ${existingTransform.transform.name}`,
                        `at ${newRoot === '.' ? 'the root.' : newRoot}`);
                });
            });
...
internals.applyTransform = function (transformObj, route, server) {
    ...
    // Prep min-clone of route without consumed properties
    let consumedRoute = route;
    for (let i = 0; i < transform.consume.length; ++i) {
        const consumedProp = transform.consume[i];
