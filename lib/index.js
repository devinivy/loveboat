...
internals.validateTransform = function (newTransform, existingTransforms) {
    ...
    const newBefore = [].concat(newTransform.before || []).concat(newTransform.transform.before || []);
    const newAfter = [].concat(newTransform.after || []).concat(newTransform.transform.after || []);

    for (let i = 0; i < existingTransforms.length; ++i) {

        const existingTransform = existingTransforms[i];

        const existingBefore = [].concat(existingTransform.before || []).concat(existingTransform.transform.before || []);
        const existingAfter = [].concat(existingTransform.after || []).concat(existingTransform.transform.after || []);
